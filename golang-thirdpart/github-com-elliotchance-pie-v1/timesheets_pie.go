...
func (ss TimeSheets) All(fn func(value TimeSheet) bool) bool {
...
func (ss TimeSheets) Any(fn func(value TimeSheet) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss TimeSheets) Append(elements ...TimeSheet) TimeSheets {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(TimeSheets{}, ss...)
...
func (ss TimeSheets) Bottom(n int) (top TimeSheets) {
...
func (ss TimeSheets) Contains(lookingFor TimeSheet) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss TimeSheets) Diff(against TimeSheets) (added, removed TimeSheets) {
	...
	diffOneWay := func(ss1, ss2raw TimeSheets) (result TimeSheets) {
		ss2 := make(TimeSheets, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss TimeSheets) DropTop(n int) (drop TimeSheets) {
	if n < 0 || n >= len(ss) {
		return
	}
...
func (ss TimeSheets) DropWhile(f func(s TimeSheet) bool) (ss2 TimeSheets) {
	ss2 = make([]TimeSheet, len(ss))
	copy(ss2, ss)
...
//	    car.Color = "Red"
//	})
func (ss TimeSheets) Each(fn func(TimeSheet)) TimeSheets {
	for _, s := range ss {
		fn(s)
	}
...
func (ss TimeSheets) Equals(rhs TimeSheets) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss TimeSheets) Extend(slices ...TimeSheets) (ss2 TimeSheets) {
	ss2 = ss
...
func (ss TimeSheets) Filter(condition func(TimeSheet) bool) (ss2 TimeSheets) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss TimeSheets) FilterNot(condition func(TimeSheet) bool) (ss2 TimeSheets) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss TimeSheets) FindFirstUsing(fn func(value TimeSheet) bool) int {
...
func (ss TimeSheets) First() TimeSheet {
	return ss.FirstOr(TimeSheet{})
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss TimeSheets) FirstOr(defaultValue TimeSheet) TimeSheet {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss TimeSheets) Float64s() pie.Float64s {
	l := len(ss)
...
func (ss TimeSheets) Insert(index int, values ...TimeSheet) TimeSheets {
	if index >= ss.Len() {
		return TimeSheets.Extend(ss, TimeSheets(values))
	}

	return TimeSheets.Extend(ss[:index], TimeSheets(values), ss[index:])
}

// Ints transforms each element to an integer.
func (ss TimeSheets) Ints() pie.Ints {
	l := len(ss)
...
func (ss TimeSheets) Join(glue string) (s string) {
	var slice interface{} = []TimeSheet(ss)
...
func (ss TimeSheets) JSONBytes() []byte {
	if ss == nil {
		return []byte("[]")
	}
...
func (ss TimeSheets) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss TimeSheets) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss TimeSheets) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss TimeSheets) Last() TimeSheet {
	return ss.LastOr(TimeSheet{})
}

// LastOr returns the last element or a default value if there are no elements.
func (ss TimeSheets) LastOr(defaultValue TimeSheet) TimeSheet {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss TimeSheets) Map(fn func(TimeSheet) TimeSheet) (ss2 TimeSheets) {
	if ss == nil {
		return nil
	}
...
func (ss TimeSheets) Mode() TimeSheets {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[TimeSheet]int)
	for _, s := range ss {
		values[s]++
	}
...
//	}
func (ss *TimeSheets) Pop() (popped *TimeSheet) {
...
func (ss TimeSheets) Random(source rand.Source) TimeSheet {
	n := len(ss)
...
//
//	ss.Sort().Reverse()
func (ss TimeSheets) Reverse() TimeSheets {
...
func (ss TimeSheets) Send(ctx context.Context, ch chan<- TimeSheet) TimeSheets {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss TimeSheets) SequenceUsing(creator func(int) TimeSheet, params ...int) TimeSheets {
	var seq = func(min, max, step int) (seq TimeSheets) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(TimeSheets, lenght)
...
func (ss TimeSheets) Shift() (TimeSheet, TimeSheets) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss TimeSheets) Shuffle(source rand.Source) TimeSheets {
	n := len(ss)
...
func (ss TimeSheets) SortStableUsing(less func(a, b TimeSheet) bool) TimeSheets {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
// slice returned will be reallocated as to not modify the input slice.
func (ss TimeSheets) SortUsing(less func(a, b TimeSheet) bool) TimeSheets {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
//
//	fmt.Sprintf("%v")
func (ss TimeSheets) Strings() pie.Strings {
	l := len(ss)
...
func (ss TimeSheets) SubSlice(start int, end int) (subSlice TimeSheets) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]TimeSheet, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]TimeSheet, end-start)
		subSlice = zeroArray[:]
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss TimeSheets) Top(n int) (top TimeSheets) {
...
func (ss TimeSheets) StringsUsing(transform func(TimeSheet) string) pie.Strings {
	l := len(ss)
...
func (ss TimeSheets) Unshift(elements ...TimeSheet) (unshift TimeSheets) {
	unshift = append(TimeSheets{}, elements...)
